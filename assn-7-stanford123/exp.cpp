 ...
 * -----------------------------------
 * The FunctionExp subclass represents a Function(range) call.  The
 * implementation of eval returns a double
 */

FunctionExp::FunctionExp(string srange) {
    this->name = srange;
}

double FunctionExp::eval(EvaluationContext& context) const {
    return 0.0;
}
