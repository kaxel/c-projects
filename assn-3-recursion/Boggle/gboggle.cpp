...
static Grid<char> board;
static Grid<bool> highlights;
static int humanScore;
...
void drawBoard(int numRows, int numCols) {
   ...
   highlights.resize(numRows, numCols);
}
...
static void drawPlayerLabel(Player player, string name) {
   ...
   gwp->add(lab, gState.scoreBox[player].x,
...
static void drawOneScore(Player playerNum, int value) {
    ...
    GLabel **scoreLabel = playerNum == COMPUTER ? &computerScoreLabel : &humanScoreLabel;
    *scoreLabel = new GLabel(str);
    (*scoreLabel)->setFont(SCORE_FONT + "-" + integerToString(SCORE_FONT_SIZE));
    (*scoreLabel)->setColor(LABEL_COLOR);
    int offset = playerNum == COMPUTER ? 20 : 0;
    gwp->add(*scoreLabel, gState.scoreBox[playerNum].x + gState.scoreBox[playerNum].w - offset,
...
void recordWordForPlayer(string word, Player player) {
   ...
   if(player == HUMAN) humanScore += word.length() - 3;
   if(player == COMPUTER) computerScore += word.length() - 3;
}
...
static void drawRoundedRect (double x, double y, double width, double height,
                             double r) {
...
string boggleBoardToString() {
    string result;
    for(int i = 0; i < board.numRows(); i++) {
        for(int j = 0; j < board.numCols(); j++) {
            result += (highlights[i][j] ? ">" : " ");
            result += board[i][j];
        }
        result += "\n";
    }
...
void printBoggleBoard() {
    cout << "Points: Human: " << humanScore << ", Computer: " << computerScore << endl;
    cout << boggleBoardToString() << endl;
}
