...
void DominosaDisplay::drawBoard(const Grid<int>& board) {
    clear();
    this->board = board;
    displayedBoard.resize(3, board.numCols() * 2 - 1);
    for(int i = 0; i < board.numCols(); i++) {
        for(int j = 0; j < 2; j++) {
            string s = integerToString(this->board[j][i]);
            displayedBoard[j * 2][i * 2] = s;
        }
    }
...
void DominosaDisplay::drawCenteredNumber(int number, double cx, double cy, const string& color) {
    ...
    double ulx = cx - lab->getWidth()/2;
    double uly = cy + lab->getHeight()/2;
    lab->setColor(color);
    lab->setFont("Times-Bold-" + integerToString(kFontSizeScale * cellDimension));
    add(lab, ulx, uly);
}

string DominosaDisplay::toString() {
    string result = "";
    for(int i = 0; i < displayedBoard.numRows(); i++) {
        for(int j = 0; j < displayedBoard.numCols(); j++) {
            result += (displayedBoard[i][j].empty() ? " " : displayedBoard[i][j]);
        }
        result += "\n";
    }
    return result;
...
void DominosaDisplay::printBoard() {
    cout << toString() << endl;
    getLine();
}
