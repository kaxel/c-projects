...
void basicStructuralTests() {
    ...
    try {
         ...
         */
        {
            logInfo("These tests will check size() isEmpty() without calling extractMin().");
         ...
         */
        {
            logInfo("We're about to start calling extractMin().");
            PQueue queue;
        ...
        {
            logInfo("This next test will check whether peek() matches extractMin().");
            PQueue queue;
            ...
            while (!queue.isEmpty()) {
                ...
                checkCondition(queue.extractMin() == expected, "Value returned by peek() matches value returned by extractMin()");
            }
        }
        ...
        /* In the same vein - what happens if we dequeue from an empty queue? */
        {
            ...
            checkCondition(didThrow, "Priority queue uses 'error' when extractMin() called on empty queue.");
        }
 ...
 * in either your enqueue() or extractMin() functions.
 */
template <typename PQueue>
    ...
    try {
         ...
         */
        {
            ...
            for (char ch = 'A'; ch <= 'H'; ch++) {
                ...
                checkCondition(queue.extractMin() == expected, "Queue should yield " + expected + ".");
            }
        }
         ...
         */
        {
            ...
            for (int i = 0; i < randomValues.size(); i++) {
                checkCondition(queue.extractMin() == randomValues[i],
         ...
         */
        {
            ...
            bool isCorrect = true;
            for (int i = 0; i < randomValues.size(); i++) {
                if (queue.extractMin() != randomValues[i]) {
                    isCorrect = false;
                    break;
                }
            }
...
/* Function: mergeTests
     * ------------------------------------------------------------
     * A function that tests the merge function in your priority
     * queue by merging two queues.
     *
     * If your code is failing these tests, you probably have a bug
...
void mergeTests() {
    ...
    try {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            /* Generate 10000 values to enter and remove. */
            logInfo("Generating 5000 random strings for each queue.");
            Vector<string> strings;
            ...
            /* Enqueue them into the priority queue. */
            logInfo("Enqueuing 5000 random strings into each queue.");
            ...
            logInfo("Merging two queues");
            PQueue *queue = PQueue::merge(&queue1, &queue2);
...
void printReplInstructions() {
    ...
    cout << "   extractMin:     Dequeues the minimum element of the priority queue." << endl;
        ...
        while (true) {
            ...
            if (!command) {
                ...
                try {
                    ...
                    } else if (action == "extractMin" || action == "d") {
                        cout << queue.extractMin() << endl;
...
void displayMenu() {
    ...
    cout << "=====================================" << endl;
    cout << REPL_VECTOR << ": Manually test VectorPQueue" << endl;
    cout << TEST_VECTOR << ": Automatically test VectorPQueue" << endl;
    cout << REPL_LINKED_LIST << ": Manually test LinkedListPQueue" << endl;
    cout << TEST_LINKED_LIST << ": Automatically test LinkedListPQueue" << endl;
    cout << REPL_HEAP << ": Manually test HeapPQueue" << endl;
    cout << TEST_HEAP << ": Automatically test HeapPQueue" << endl;
    cout << REPL_EXTRA << ": Manually test BinomialHeapPQueue" << endl;
    cout << TEST_EXTRA << ": Automatically test BinomialHeapPQueue" << endl;
    cout << QUIT << ": Quit" << endl;
}
...
int main() {
    while (true) {
        ...
        case TEST_VECTOR:
            testPriorityQueue<VectorPQueue> ();
            break;
        case REPL_VECTOR:
            replTestPriorityQueue<VectorPQueue> ();
            break;
        case TEST_LINKED_LIST:
            testPriorityQueue<LinkedListPQueue> ();
            break;
        case REPL_LINKED_LIST:
            replTestPriorityQueue<LinkedListPQueue> ();
            break;
        case TEST_HEAP:
            testPriorityQueue<HeapPQueue> ();
            break;
        case REPL_HEAP:
            replTestPriorityQueue<HeapPQueue> ();
            break;
        case TEST_EXTRA:
            testPriorityQueue<BinomialHeapPQueue> ();
            break;
        case REPL_EXTRA:
            replTestPriorityQueue<BinomialHeapPQueue> ();
            break;
        case QUIT:
            return 0;
        default:
