...
/* Type: AlgorithmType
 *
 * An enumerated type representing one of Dijkstra's algorithm or A* search (added A* bidirectional).
 */
enum AlgorithmType {
  DIJKSTRA, A_STAR, BI_DIR
};
...
const string kBiDir("Bi-directional ");
...
static double runShortestPath(Grid<double>& world, 
   ...
   */
    switch(algType) {
        case A_STAR:
            path = invoke(shortestPath, start, end, world, costFn, hFn);
            break;
        case BI_DIR:
            path = invoke(shortestPathBiDirectional, start, end, world, costFn, hFn);
            break;
        case DIJKSTRA:
            path = invoke(shortestPath, start, end, world, costFn, zeroHeuristic);
            break;
    }
